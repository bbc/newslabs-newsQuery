...
module.exports = function(apiKey) {
    ...
    this.getConceptOccurrencesOverTime = function(conceptUri, startDate, endDate) {
        ...
        if (startDate == endDate) {
            ...
            for (var i = 0; i < 365; i++) {
                var date = moment(startDate, 'YYYY-MM-DD').add('days', i).format('YYYY-MM-DD');
                if (date == endDate)
                    break;
                dates.push(date);
            }
        ...
        dates.forEach(function(date) {
            var after = date;
            var before = moment(date, 'YYYY-MM-DD').add('days', 1).format('YYYY-MM-DD');
            ...
            .then(function(response) {
                if (response['co-occurrences'] && response['co-occurrences'][0].occurrence) {
                    ...
                    return "An error has occured. Your API key may be invalid. Or the Force may not be with you today.";
                }
            });
            promises.push(promise);
        });
