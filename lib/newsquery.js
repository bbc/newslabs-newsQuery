...
module.exports = function(apiKey, cacheEnabled, cacheOptions) {
    ...
    /**
     ...
     this.getConcept = function(conceptUri, articleLimit) {
         ...
         var cacheId = crypto.createHash('sha1').update( JSON.stringify({ method: getConcept, conceptUri: conceptUri, articleLimit: articleLimit}) ).digest("hex");         
    ...
    this.getConcepts = function(keywords, limit, conceptTypeUris) {
        ...
        var cacheId = crypto.createHash('sha1').update( JSON.stringify({ method: getConcepts, keywords: keywords, limit: limit, conceptTypeUris: conceptTypeUris }) ).digest("hex");         
        var cacheValue = this.cacheRead(cacheId);
        if (cacheValue !== false)
            return cacheValue;
    ...
    this.getCoOccuringConcepts = function(conceptUri, limit, conceptTypeUris, sourceUri) {
        ...
        var cacheId = crypto.createHash('sha1').update( JSON.stringify({ cmethod: getCoOccuringConcepts, onceptUri: conceptUri, limit: limit, conceptTypeUris: conceptTypeUris, sourceUri: sourceUri }) ).digest("hex");         
        var cacheValue = this.cacheRead(cacheId);
        if (cacheValue !== false)
            return cacheValue;
    ...
    this.getArticlesByConcept = function(conceptUris, limit, offset) {
        ...
        var cacheId = crypto.createHash('sha1').update( JSON.stringify({ method: getArticlesByConcept, conceptUris: conceptUris, limit: limit, offset: offset }) ).digest("hex");         
        var cacheValue = this.cacheRead(cacheId);
        if (cacheValue !== false)
            return cacheValue;
    ...
    this.getConceptOccurrences = function(conceptUri) {
        ...
        var cacheId = crypto.createHash('sha1').update( JSON.stringify({ method: getConceptOccurrences, conceptUri: conceptUri }) ).digest("hex");         
        var cacheValue = this.cacheRead(cacheId);
        if (cacheValue !== false)
            return cacheValue;
    ...
    this.getConceptOccurrencesOverTime = function(conceptUri, startDate, endDate, sourceUri) {
        ...
        var cacheId = crypto.createHash('sha1').update( JSON.stringify({ method: getConceptOccurrencesOverTime, conceptUri: conceptUri, startDate: startDate, endDate: endDate, sourceUri: sourceUri }) ).digest("hex");         
        var cacheValue = this.cacheRead(cacheId);
        if (cacheValue !== false)
            return cacheValue;
    ...
    /**
     * @todo Ass support for different arguments
     * This method is new and experimental!
     * @todo Add support for date ranges, sources, pagination and other options
     */
    this.searchArticles = function(keywords) {
        var cacheId = crypto.createHash('sha1').update( JSON.stringify({ keywords: keywords}) ).digest("hex");
        var cacheValue = this.cacheRead(cacheId);
        if (cacheValue !== false)
            return cacheValue;
            //this.juicerApiHost/articles.json?text=London&product[]=TheGuardian&product[]=NewsWeb&content_format[]=TextualFormat&section[]=China&site[]=&published_after=2010-10-10&published_before=2014-01-01&apikey={{apikey}}
        ...
        // 
        url += "&published_after="+moment().subtract(6, 'months').format('YYYY-MM-DD');
        url += "&product[]=NewsWeb";
