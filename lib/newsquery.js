...
module.exports = function(apiKey) {
    ...
    /**
      * Get info about t concept by it's URI, including articles mentioning it
      *
     ...
     * @param    {Number}   limit               Set a limit on the number of results to return
     */
    this.getRelatedConcepts = function(conceptUri, conceptTypeUris, limit) {
        if (limit === void(0))
            limit = 10;
            
        var url = this.semanticNewsApiHost+'/concepts/co-occurrences/?uri='+encodeURIComponent(conceptUri)+'&limit='+encodeURIComponent(limit)+'&apikey='+encodeURIComponent(this.apiKey);
        ...
        .then(function(response) {
            ...
            var result = [];
            for (var i=0; i<response['co-occurrences'].length; i++) {
                // Skip concepts that are the same as the concept we searched for
                if (conceptUri == response['co-occurrences'][i].thing)
                    continue;
                result.push({   name: response['co-occurrences'][i].label,
                                uri: response['co-occurrences'][i].thing,
                                occurrences: response['co-occurrences'][i].occurrence
                            });
            }
            return result;
        });
